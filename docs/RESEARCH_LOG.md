...
## 2026-08-07 — 인원 축 zero-shot 재설계 (고정 인원 8/48/60, n=40)

구 K-scaling(비율 K/2..K, n=8, pilot/test 겸용, baseline 2종)이 §2 그리드와
프로토콜이 어긋나 폐기하고, 사용자 재설계안으로 재실행: **학습 세계 그대로
+ 유저 수만 고정**(전원 활성), 40k pilot → 30k n=40, baseline 6종.

**결과**: K8 **+104±34 (37/40, CI>0)**, 챔피언 SU+CQI 3311, PPO depth 1.91 /
K48 **+377±99 (38/40, CI>0)**, 챔피언 SUS+CQI 1217, depth 3.40 / K60
**+144±148 (26/40, tie)**, 전원 음수 지대, depth 3.71.

**판독**: ①학습 범위(16–32) 절반·1.5배에서 확정 승, ~1.9배에서 동급 —
붕괴 없음 ②**depth 자가 적응 1.91→3.40→3.71 단조** (동일 가중치 파일,
재학습 0) = K-독립 구조의 기능적 증거 ③챔피언 교체 재현(K8만 SU+CQI) —
P010 저부하 패턴과 동일 ④rate-blind는 K48+에서 −10,000 이하로 붕괴
⑤K60 tie는 인구 외삽 실패가 아니라 ρ≈3.0 포화(P065류)의 결과.
주의: 인원↑=부하↑ 결합(ρ 0.4/2.4/3.0), K 간 비paired.

## 2026-08-07 (2) — ⭐메인 결과 held-out n=40 확정: +15.2%, 40/40 전승, t=15.33

사용자 지시로 held-out을 예약 대역 전체(10000–10039)로 확장하고, 동시에
**threshold 선택을 미사용 40000–40007로 분리**(구 final20은 10000–10007에서
선택 후 같은 시드로 평가 = OOD에서 고친 누출이 주 결과에도 남아 있었음).
이제 held-out 40시드 전부가 어떤 튜닝 결정과도 무관. 신 T*=0.75
(구 0.7; pilot 4418 vs 4411로 사실상 동률 — 곡선 평탄 재확인).

**결과 (n=40, SUS+CQI@0.75)**: **PPO 4749 vs 4124 = +15.2%, paired
+625±82 (CI>0), t=15.33, 챔피언 대비 40/40 전승, seed-best 대비 39/40**.
depth 2.72 vs 3.92 / miss 0.356 vs 0.386 / goodput 78.3 vs 75.4 /
JFI 0.689 vs 0.697.

**구간 분리 확인**: 구 20시드 +612 (20/20), **신규 20시드 +638 (20/20)** —
신규 구간에서 오히려 소폭 큼. n=20 시절 +13.9%가 시드 운이 아니었음이
확정되고, 마진도 +13.9%→+15.2%로 상승(누출 제거로 baseline이 자기 시드
이점을 잃은 효과와 정합).

전 baseline 서열: PPO 4749 > SUS+CQI 4124 > SU+CQI 2061 > SUS+PF 1332 ≈
...
## 2026-08-10 (2) — ⭐OOD 전체 n=100 확장 완료: 8/9 확정 승 + 설계상 tie 1

시드 30040–30099(+60)를 9세계 전체에 균일 확장(그리드 GPU1–5, 인원축
GPU3/4/5 시드분할). T* 고정(40k pilot), D26 strict 유지 — 순수 표본 확장.

**n=100 최종 판정**: P055 +641±86(95/100) / P010 +13±14 tie(설계상;
하한 −0.1로 경계) / V60max +623±61(96/100) / CSI02 +517±51(97/100) /
**D26 +839±68(99/100)** / STORM2 +532±83(93/100) / K8 +124±27(95/100) /
K48 +364±56(92/100) / **K60 +185±76 — n=40 tie에서 CI>0 확정 승격**
(73/100; ρ≈3.0 포화 지대에서도 유의한 우위).

병합 데이터 `ood_all_n100.csv`(6,300행 = 9세계×7종×100시드). README
§2/§3 갱신. 각주: n=40 사전등록 판정 전부 유지, 확장은 결과-무관 일괄.
이로써 논문의 모든 평가(메인 held-out, OOD 그리드, 인원 축)가 n=100으로
통일 완료.
