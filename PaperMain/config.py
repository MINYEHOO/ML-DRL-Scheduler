...
class Config:
    ...
    # ---- traffic (random arrivals, finite per-UE FIFO queues) ----
    # "ftp3" selects FTP Model 3's per-UE Poisson arrival process only;
    # packet sizes, deadlines and buffer rules retain this experiment's values.
    traffic_model: str = "bernoulli"  # "bernoulli" | "ftp3"
    # Bernoulli: arrival probability per UE per slot. FTP3: Poisson mean
    # packets per UE per slot (lambda [packets/s] = p_arrival / slot_duration).
    # The same numeric value preserves mean offered arrivals, not P(N >= 1).
    p_arrival: float = 0.2
    ...
    # In Bernoulli mode, queue_size = 1 reproduces Run3 BIT-EXACTLY
    ...
    # FTP3 always samples arrivals, even when busy; every rejected packet
    # counts as overflow, including with queue_size = 1.
    queue_size: int = 1

    def validate_traffic(self) -> None:
        """Reject invalid traffic settings before the environment is started."""
        if self.traffic_model not in ("bernoulli", "ftp3"):
        ...
        for name in ("p_arrival", "p_arrival_min", "p_arrival_max"):
            ...
            if (isinstance(value, (bool, np.bool_))
                    ...
                    or (self.traffic_model == "bernoulli" and value > 1)):
                unit = "probability in [0, 1]" if self.traffic_model == "bernoulli" else "finite nonnegative mean packets/slot"
                raise ValueError(f"{name} must be a {unit}, got {value!r}")
        if self.p_arrival_max > 0 and self.p_arrival_min > self.p_arrival_max:
            raise ValueError("p_arrival_min must not exceed p_arrival_max")
        if (isinstance(self.queue_size, (bool, np.bool_))
                or not isinstance(self.queue_size, (int, np.integer))
    ...
    def describe(self) -> str:
        ...
        lines = [
            ...
            + (f"mean_arrivals_per_ue_slot={self.p_arrival}, "
               if self.traffic_model == "ftp3" else
               f"p_arrival={self.p_arrival}, ") +
