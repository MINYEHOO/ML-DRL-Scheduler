#!/usr/bin/env python3
"""Raw-input (encoder-free, phase-invariant pooling) recipe runner; GPU 3.
...
NAMES = {'smoke': '29_audit_raw_invariant_gpu3',
         'train': '30_raw_invariant_lr1000_s2024_20260916_gpu3'}
GPU = 3
...
EXTENSIONS = ['paper_train_raw.py', 'artifacts/raw_invariant/config.json',
              'review/runners/run_raw_invariant.py']
...
def command(mode):
    c = [sys.executable, '-u', 'paper_train_raw.py', '--recipe', pr.RECIPE, '--name', NAMES[mode],
...
def prepare(mode):
    ...
    expected = dict(ref['config'], seed=2024)
    if mode == 'smoke':
    ...
    assert checkpoint is None and m['schedule'] == SCHEDULE and m['recipe'] == pr.RECIPE
...
def supervise(mode):
    ...
    try:
        ...
        if mode == 'smoke':
            phases.append(([sys.executable, '-u', 'paper_train_raw.py', '--recipe', pr.RECIPE,
