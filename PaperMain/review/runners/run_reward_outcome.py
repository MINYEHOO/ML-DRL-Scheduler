#!/usr/bin/env python3
"""Outcome-only reward recipe runner; GPU 2.

Mirrors review/runners/run_reward_c2m6.py. The audit run is unnumbered and is
deleted once the real launch is dispatched (user rule 2026-09-16); its
preflight/launch JSONs stay in review/logs as evidence.

    python review/runners/run_reward_outcome.py smoke --preflight
    python review/runners/run_reward_outcome.py smoke
    python review/runners/run_reward_outcome.py train --preflight
    python review/runners/run_reward_outcome.py train
"""
...
NAMES = {'smoke': 'audit_reward_outcome_gpu2',
         'train': '33_reward_outcome_lr1000_s2024_20260916_gpu2'}
GPU = 2
...
EXTENSIONS = ['paper_train_outcome.py', 'artifacts/reward_outcome/config.json',
              'review/runners/run_reward_outcome.py']
...
def command(mode):
    c = [sys.executable, '-u', 'paper_train_outcome.py', '--recipe', po.RECIPE, '--name', NAMES[mode],
...
def prepare(mode):
    c = command(mode)
    run, checkpoint, m = po.plan(po.parser().parse_args(c[3:]), ROOT)
    ...
    expected = dict(ref['config'], seed=2024, **po.NEW_WEIGHTS)
    if mode == 'smoke':
    ...
    assert checkpoint is None and m['schedule'] == SCHEDULE and m['recipe'] == po.RECIPE
    ...
    assert m['variant_provenance']['reward'] == po.REWARD
    return dict(command=c, run_dir=str(run), resume=None, **m,
                ...
                same_world_as_base=True, reward_design=po.REWARD)
...
def supervise(mode):
    ...
    try:
        ...
        if mode == 'smoke':
            phases.append(([sys.executable, '-u', 'paper_train_outcome.py', '--recipe', po.RECIPE,
        ...
        for c, total in phases:
            ...
            assert ck['cfg']['lambda_s'] == 0.0 and ck['cfg']['lambda_m'] == 1.0 and ck['cfg']['lambda_c'] == 1.0
