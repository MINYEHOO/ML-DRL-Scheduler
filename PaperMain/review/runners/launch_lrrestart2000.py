...
name = '16_base_cqi4_lrrestart2000_s2024_20260910_gpu5'
run = root/'runs'/name
...
assert pre['continuation'] == lineage and lineage['next_update'] == 1000
assert pre['schedule'] == {'lr_final': 0.0, 'lr_decay_updates': 1000, 'lr_initial': 1e-5, 'lr_start_update': 1000}
assert pre['target_updates'] == 2000 and pre['config']['seed'] == 2024
...
assert lineage['resumed_checkpoint_update'] == 999
assert lineage['continuation_schedule'] == pre['schedule']
...
command = [sys.executable, '-u', 'paper_train.py', '--recipe', 'base',
           '--resume', str(run/'ckpt/latest.pt'), '--num-updates', '2000']
...
state = dict(status='starting', name=name, run_dir=str(run), console_log=str(console), command=command,
             ...
             device='cuda', physical_gpu=5, seed=2024, target_updates=2000, slots=1000,
             beta=pre['config']['la_beta_by_depth'], lr_schedule=pre['schedule'],
             initial_lr=pre['config']['ppo_learning_rate'], continuation_initial_lr=1e-5, replay_mode='batched',
             eval_action_only=True, parent_run=lineage['parent_run'], start_update=1000,
             resumed_checkpoint_update=999, target_completed=False)
