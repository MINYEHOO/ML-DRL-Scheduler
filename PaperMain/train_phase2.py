...
def parse_args():
    ...
    p.add_argument("--traffic_model", choices=("bernoulli", "ftp3"), default=None,
                   help="arrival process; FTP3 draws Poisson(p_arrival) counts per UE per slot")
    ...
    p.add_argument("--lr_final", type=float, default=None,
                   help="anneal the Adam learning rate linearly from "
                        "cfg.ppo_learning_rate (or --lr_initial) to this value "
                        "over --lr_decay_updates, then hold")
    p.add_argument("--lr_decay_updates", type=int, default=None,
                   help="updates to reach --lr_final (then held); "
                        "default = num_updates")
    p.add_argument("--lr_initial", type=float, default=None,
                   help="initial LR of a documented continuation segment; "
                        "requires --lr_start_update, --lr_final and --lr_decay_updates")
    p.add_argument("--lr_start_update", type=int, default=None,
                   help="global update index at the start of --lr_initial's segment")
...
# helpers
# ---------------------------------------------------------------------------
def linear_learning_rate(initial_lr: float, final_lr: float,
                         decay_updates: int, update: int,
                         start_update: int = 0) -> float:
    """LR from the global update index; restarting a process never restarts decay."""
    if (isinstance(initial_lr, bool) or isinstance(final_lr, bool)
            or not isinstance(initial_lr, (float, int)) or not isinstance(final_lr, (float, int))
            or not math.isfinite(initial_lr) or not math.isfinite(final_lr)
            or initial_lr <= 0 or not 0 <= final_lr <= initial_lr):
        raise ValueError("invalid linear learning-rate endpoints")
    if (isinstance(decay_updates, bool) or not isinstance(decay_updates, int)
            or decay_updates <= 0):
        raise ValueError("LR decay_updates must be a positive integer")
    if (isinstance(start_update, bool) or not isinstance(start_update, int)
            or start_update < 0 or isinstance(update, bool) or not isinstance(update, int)
            or update < start_update):
        raise ValueError("update precedes the LR segment start or has an invalid index")
    frac = min((update - start_update) / decay_updates, 1.0)
    return initial_lr + (final_lr - initial_lr) * frac
    ...
    def schedule(self, env) -> np.ndarray:
        with torch.no_grad():
            if self.deterministic:
                return self.ac.deterministic_action(env.get_observation())
...
def run_eval(env, ac, cfg: Config, update: int,
    ...
    for sched in schedulers:
        ...
        writer.add_scalar(f"timing/eval/{sched.name}_seconds", eval_seconds, update)
        summary[sched.name] = rew
    total_eval_seconds = time.perf_counter() - eval_started
    writer.add_scalar("timing/eval_total_seconds", total_eval_seconds, update)
    print(f"  [eval-time] update {update}: {total_eval_seconds:.1f}s total")
    return summary["PPO"]
...
def main(config_override=None):
    ...
    # PaperMain restores the complete frozen configuration before constructing
    # the environment/model. The historical CLI remains intact.
    if config_override is not None:
        cfg = copy.deepcopy(config_override)
        args.seed = cfg.seed
        cfg.validate_la()
    cfg.validate_traffic()
    if args.resume:
        saved_traffic = torch.load(args.resume, map_location='cpu', weights_only=False).get('cfg', {}).get('traffic_model', 'bernoulli')
        if saved_traffic != cfg.traffic_model:
            raise ValueError('traffic_model cannot change on resume; start a fresh run')
    num_updates = args.num_updates or default_updates
    # Keep the legacy two-field schedule representation. A continuation adds
    # its explicit starting LR and global origin without changing cfg or the
    # state restored from the parent checkpoint.
    from paper_train import validate_schedule, validate_checkpoint_schedule
    lr_schedule = dict(lr_final=args.lr_final,
                       lr_decay_updates=(args.lr_decay_updates if args.lr_decay_updates is not None else num_updates)
                       if args.lr_final is not None else None)
    if (args.lr_initial is None) != (args.lr_start_update is None):
        raise ValueError("--lr_initial and --lr_start_update must be supplied together")
    if args.lr_initial is not None:
        if args.lr_final is None or args.lr_decay_updates is None:
            raise ValueError("LR segment requires --lr_final and --lr_decay_updates")
        lr_schedule.update(lr_initial=args.lr_initial, lr_start_update=args.lr_start_update)
    validate_schedule(lr_schedule, cfg.ppo_learning_rate)
    if not args.resume and lr_schedule.get("lr_start_update", 0) != 0:
        raise ValueError("a nonzero LR segment start requires a continuation checkpoint")
    ...
    if args.resume:
        ...
        validate_checkpoint_schedule(ckpt, lr_schedule)
    ...
    def ckpt_payload(update_idx: int) -> dict:
        ...
        payload = {"model": {k: v.detach().clone()
                   ...
                   "lr_schedule": copy.deepcopy(lr_schedule),
                   "cfg": cfg_dump}
    ...
    lr0 = lr_schedule.get("lr_initial", cfg.ppo_learning_rate)
    lr_start = lr_schedule.get("lr_start_update", 0)
    if args.lr_final is not None:
        lr_horizon = lr_schedule["lr_decay_updates"]
        print(f"lr anneal: {lr0} -> {args.lr_final} over {lr_horizon} "
              f"updates from global update {lr_start} (linear, resume-safe)")
        ...
        for update in range(start_update, num_updates):
            ...
            if args.entropy_coef_final is not None:
                ...
                cfg.ppo_entropy_coef = (ent_coef0 +
                                        ...
                                        * frac)
            if args.lr_final is not None:
                new_lr = linear_learning_rate(lr0, args.lr_final, lr_horizon,
                                              update, start_update=lr_start)
