...
- [x] ~~배치 replay 다지점 ON/OFF 등가성~~ → **완료** (§9b).
- [x] ~~배치 replay 자기 재현성~~ → **완료, 비트 단위 동일** (§9b).
...
**다지점 등가성 (2026-08-24).** 등가성이 한 가중치 지점의 우연이 아님을 학습 단계가
다른 체크포인트 셋에서 재확인했다 (`tests/results/equiv_*.log`):

| 체크포인트 | 단계 | 단일-스텝 게이트 | 7개 지표 최악 |
|---|---|---|---|
| s5024 @389 | 초중반 | 6.57e-06 | 7.29e-06 |
| s3024 @489 | 중반 | 1.68e-05 | 3.45e-06 |
| s4024 @849 | 거의 끝 | 7.73e-06 | 1.23e-05 |
| 2024 @409 | (기존) | 6.81e-05 | 6.90e-07 |

**자기 재현성 (2026-08-24) — 비트 단위 동일.** `replay_batch`의
`e.permute(0,2,1,3)[slot_t, r_t]`는 한 RBG의 4개 레이어가 같은 `(slot, r)`을 가리키는
many-to-one 인덱싱이라, 역전파가 CUDA `atomicAdd`를 타면 같은 시드로 두 번 돌려도
결과가 달라질 수 있다는 우려가 있었다. **측정이 부정했다**: 동일 시드·동일 명령으로
배치 ON 3 update를 두 번 돌려 `env_metrics` 18건 + `ppo_metrics` 21건을 대조한 결과
**불일치 0건**이다 (`tests/results/determinism_batched.log`). PyTorch가 이 인덱싱의
역전파에 결정적 경로를 쓴다. 따라서 플래그 ON에서도 `(시드, 코드, update 수) → 하나의
답`이 유지되고 `--resume`도 중단 없이 돌린 것과 같은 궤적을 낸다. gather를 제거하는
대안(ScoreNet을 `[B,R,K,71]` 격자 전체에 돌려 14% 낭비를 감수)은 불필요하다.
