...
class TrafficModel:
    """Per-UE packet queue + Bernoulli arrival process.
    ...
    def generate_arrivals(self, slot: int, rng: np.random.Generator):
        """Bernoulli arrivals. Returns (new_ue_list, n_buffer_overflow).

        queue_size == 1: the Run3 path, RNG-EXACT -- the Bernoulli draw
