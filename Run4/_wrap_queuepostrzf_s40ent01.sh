#!/bin/bash
# QueuePostRZF_S40Ent01 -- S40Ent01 = S40Ent02 fork @best.pt(upd519) with entropy 0.02->0.01 (anneal-to-commit, user directive 2026-07-26).
# QueuePostRZF world with FOUR deltas: (1) entropy 0.02 FROM SCRATCH (fresh,
# not fork -- clean-pipeline test), (2) speed U(5,40), (3) beta_m recalibrated
# for S40 = 0.9757,0.7180,0.6355,0.5837 (beta_m_calib_queue_s40, holdout
# first-ACK 0.89-0.95), (4) save_every=1 + extended eval CSV (new pin).
...
RUN_DIR=Run4/QueuePostRZF_S40Ent01
LOG=Run4/QueuePostRZF_S40Ent01_run.log
...
for i in $(seq 1 1000); do
  ...
  if [ -f "$RUN_DIR/ckpt/latest.pt" ]; then
    ...
    ARGS="--run_root Run4 --run_name QueuePostRZF_S40Ent01"
  fi
