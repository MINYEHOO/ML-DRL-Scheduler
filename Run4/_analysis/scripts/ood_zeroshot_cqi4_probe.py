"""OOD zero-shot probe, CQI4 edition: frozen 4-bit-CQI policy in worlds
OUTSIDE its training distribution (mirror of ood_zeroshot_probe.py,
user-requested 2026-08-04).

Policy = QueuePostRZF_S40HL_CQI4 best.pt@409 (held-out +13.9%, 20/20),
frozen, no retraining. Training distribution: cqi_mode='nr4bit',
speed U(5,40) km/h, p_arrival U(0.15,0.50), p_csi 0.6, K=32 active U(16,32).
All OOD worlds KEEP cqi_mode='nr4bit' and the CQI4 beta_m tuple
(1.0018, 0.7499, 0.6592, 0.6058) -- zero-shot premise, K48-style footnote.

Configs (identical grid to the 2026-07-27 probe; each changes ONLY the
listed knobs):
  P065  p fixed 0.65  -- deep load extrapolation. ALSO evaluates the frozen
        continuous-CQI policy (S40HighLoad best@639) on the same nr4bit
        seeds: the report-granularity adaptation-gap pair (analogous to
        the Ent02 pair in the original probe).
  V50   speed fixed 50 km/h
  V60   speed fixed 60 km/h
  P055  p fixed 0.55
  CSI04 p_csi 0.4
  STORM speed U(40,60) + p fixed 0.55 + p_csi 0.5

Protocol: seeds 30000-30007 (8; 20000+ reserved for final paper numbers).
Per config, SUS+CQI threshold re-swept over {0.60..0.80} on the full 8
seeds; best baseline = max(best SUS+CQI, SU+CQI); wins counted paired.
8-seed verdict rule: low single-digit margins read as "does not lose".
"""
import os, sys, json, time
# CPU by default; honour an externally set CUDA_VISIBLE_DEVICES (e.g. GPU 5)
...
OUT = "Run4/_analysis/ood_zeroshot_cqi4.csv"
...
CK_HL = torch.load("Run4/QueuePostRZF_S40HighLoad/ckpt/best.pt", map_location="cpu")
...
for cname, overrides in CONFIGS:
    ...
    # frozen policies (continuous-CQI HL pair only on P065)
    policies = [("PPO-CQI4", CK)] + ([("PPO-HL(cont)", CK_HL)] if cname == "P065" else [])
...
print("\n=== OOD zero-shot 요약 (CQI4 best@409 동결, nr4bit 세계, seeds 30000-30007) ===")
...
print("참고: 원본(continuous) 6-세계 결과는 ood_zeroshot.out — 마진 축 비교용.")
print("saved ->", OUT)
f.close()
