"""CSI01+V60 combined stress probe (user-requested 2026-08-06): p_csi = 0.1
AND speed fixed 60 km/h, nr4bit world, CQI4 policy only. Mean CSI age ~4.5 ms
vs coherence ~2.2 ms at 60 km/h -> age/Tc ~ 2: directional CSI mostly expired.

Motivation: the trained p_csi = 0.6 (mean report interval 0.83 ms) is
generous vs real CSI periodicity (5-20 ms). p_csi = 0.1 gives a mean
report interval of 5 ms and mean CSI age ~2 ms -- much closer to a
dense real reporting configuration (age/coherence ~1.0 at 30 km/h -- the deep-stale regime of real 10 ms periodic reporting).
...
zero-shot premise, the LA margin is knowingly mis-calibrated here):
  CSI01-cont : continuous-CQI world, frozen S40HighLoad best.pt@639
  CSI01-cqi4 : nr4bit world,        frozen S40HL_CQI4 best.pt@409
...
OUT = "Run4/_analysis/ood_csi01v60.csv"
SEEDS = list(range(30000, 30008))
SUS_THRS = [0.60, 0.65, 0.70, 0.75, 0.80]

EDITIONS = [
    ("CSI01V60-cqi4", "Run4/QueuePostRZF_S40HL_CQI4"),
]
...
def make_cfg(base):
    ...
    raw["p_csi"] = 0.1
    raw["ue_speed_min"] = 60.0
    raw["ue_speed_max"] = 60.0
    return Config(**raw)
...
for cname, base in EDITIONS:
    ...
    print(f"\n=== [{cname}] base {base}, p_csi 0.1, policy best.pt@{ck['update']} ===", flush=True)
    cfg = make_cfg(base)
    assert cfg.p_csi == 0.1
...
print("\n=== CSI01+V60 (p_csi 0.1 + 60 km/h) zero-shot 요약, seeds 30000-30007 ===")
