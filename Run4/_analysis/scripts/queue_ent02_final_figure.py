"""QueuePostRZF_Ent02 final-metrics figure -- PPO vs SUS-MU vs SU baselines.

Same 8-panel format as the L2b / GenieFineTune figures: 20 held-out seeds
(10000-10019), every scheduler on every seed, showcase = best-margin
episode. World = queue post-RZF beta_m (the LA-surrogate world); PPO =
Ent02 best.pt@769 (dose-response winner). SUS threshold passed via
QSUS_THR env var (from queue_sus_threshold_sweep).
"""
...
ac.eval()
print(f"Ent02 best.pt update {ck['update']} run-eval {ck.get('eval_reward', float('nan')):.1f}; SUS thr {THR}", flush=True)
...
# queue mode ships 12 baselines (adds EDF/MW variants); keep the standard
# figure set of 8. Safe: on the betam 8-seed grid SUS+MW (4245) < SUS+CQI
# (4819) and EDF variants are negative, so no stronger baseline is dropped.
...
with open(f"{OUTDIR}/queue_ent02_final20.csv", "w", newline="") as f:
    w = csv.writer(f)
...
fig.suptitle(f"QueuePostRZF_Ent02 — PPO vs baselines  (seed {show} best-margin;  "
             f"SUS thr {THR} in-world swept;  best.pt@{ck['update']};  "
             f"post-RZF + beta_m LA-surrogate world)", fontsize=10)
fig.text(0.5, 0.945,
         f"K = {cfg.num_ue} UEs ({na} active)  ·  per-UE speeds (km/h, sorted): "
         + " ".join(f"{s:.1f}" for s in spd), ha="center", fontsize=6.5)
fig.tight_layout(rect=(0, 0, 1, 0.93))
png = f"Run4/QueuePostRZF_Ent02/QueueEnt02_metrics_seed{show}.png"
