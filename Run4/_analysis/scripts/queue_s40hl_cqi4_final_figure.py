"""QueuePostRZF_S40HL_CQI4 final-metrics figure -- retired-run held-out eval.

Same protocol as the S40HighLoad / GenieS40HL_FineTune finals, for the retired
4-bit-CQI arm (best.pt@409, 5038.9 run-eval; world = S40HighLoad queue
post-RZF, speed U(5,40), p U(0.15,0.50), p_csi 0.6, cqi_mode='nr4bit' 16-entry
TS 38.214 Table 5.2.2.1-3 floor-snap, beta_m = (1.0018, 0.7499, 0.6592,
0.6058) recalibrated for this world):
(1) sweep SUS+CQI threshold in THIS world (8 seeds) -> strongest T*
    (the 0.75 in config was inherited from the continuous-CQI point, never
    re-swept -- mandatory before any verdict);
(2) evaluate PPO(best.pt@409) + 8 baselines at T* on 20 held-out seeds;
...
cfg0 = make_cfg()
assert cfg0.cqi_mode == "nr4bit", "not the 4-bit CQI world"
assert cfg0.resolved_la_mode() == "post_rzf" and cfg0.la_beta_by_depth

# --- stage 1: SUS+CQI threshold sweep (8 seeds) ---
print("=== SUS+CQI threshold sweep (S40HL nr4bit world, 8 seeds) ===", flush=True)
...
with open(f"{OUTDIR}/queue_s40hl_cqi4_final20.csv", "w", newline="") as f:
    w = csv.writer(f)
...
fig.suptitle(f"QueuePostRZF_S40HL_CQI4 — PPO vs baselines  (seed {show} best-margin;  "
             f"SUS thr {best_thr} in-world swept;  best.pt@{ck['update']};  "
             f"4-bit NR CQI (TS 38.214 Tab. 5.2.2.1-3 floor-snap), post-RZF + beta_m(CQI4), "
             f"speed U(5,40), p U(0.15,0.50))", fontsize=9)
fig.text(0.5, 0.945,
         f"K = {cfg.num_ue} UEs ({na} active)  ·  per-UE speeds (km/h, sorted): "
         + " ".join(f"{s:.1f}" for s in spd), ha="center", fontsize=6.5)
fig.tight_layout(rect=(0, 0, 1, 0.93))
png = f"{RUN}/QueueS40HLCQI4_metrics_seed{show}.png"
...
print("(reference margins, matched protocol: S40HighLoad continuous-CQI +14.7% 20/20; "
      "GenieS40HL perfect-CSI +4.3% 20/20)", flush=True)
