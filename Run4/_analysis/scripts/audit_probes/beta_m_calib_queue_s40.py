"""beta_m recalibration for the QUEUE world at speed U(5,40) (2026-07-20).

Frozen recipe (same as beta_m_calib_holdout.py / beta_m_calib_l2c.py):
random (RBG, m, group) samples over empty-allocation episodes; beta_m =
per-depth 10th pct of MI_actual/cap_pred; holdout on disjoint episodes
with the DEPLOYED 4-decimal values. World = QueuePostRZF config.json
(K=32, n_active U{16..32}, p_csi 0.6, type2 56-bit) with the speed axis
widened to U(5,40) and beta cleared (raw-prediction ratios).
For the fresh-Ent02 S40 run (user directive 2026-07-20).
"""
import os, sys, json
...
print(f"world: K={cfg.num_ue} n_active U({cfg.n_active_min},{cfg.n_active_max}) "
      f"speed U({cfg.ue_speed_min},{cfg.ue_speed_max}) p_csi={cfg.p_csi} "
      f"pmi={cfg.pmi_mode}", flush=True)
...
print("DEPLOYED beta_m(queue S40):", beta, flush=True)
