"""GenieFineTune final-metrics figure -- PPO vs SUS-MU vs SU baselines.

Recreates the MixedSpeed_L2b 8-panel bar figure (Run3/_analysis_20260702/
MixedSpeed_L2b_metrics_seed10019.png) for the retired GenieFineTune run
(perfect-CSI world, actor warm-started from L2b; best.pt@729):
held-out seeds 10000-10019, every scheduler on every seed, showcase seed =
best PPO margin over the best baseline, single-episode bars.

Panels: reward, throughput, mean SINR, completion, MU depth,
failure modes (deadline-miss + retx-drop stacked), TOTAL failure, JFI.
"""
...
ac.eval()
print(f"best.pt update {ck['update']} eval {ck.get('eval_reward', float('nan')):.1f}", flush=True)
...
data = {}      # (seed, sched) -> metrics dict
ctx = {}       # seed -> (n_active, sorted active speeds)
for seed in SEEDS:
    ...
    print(f"seed {seed} (n_active {na}): PPO {data[(seed,'PPO')]['reward']:.0f} "
          f"margin {margin:+.0f}", flush=True)

with open(f"{OUTDIR}/geniefinetune_final20.csv", "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["seed", "sched"] + KEYS)
    for (seed, name), m in data.items():
...
fig.suptitle(
    ...
    f"SUS threshold {cfg.sus_ortho_threshold};  PPO = best.pt@{ck['update']};  "
...
# 20-seed means for the report
print("\n20-seed means:")
for n in ORDER:
    mm = {k: np.mean([data[(s, n)][k] for s in SEEDS]) for k in KEYS}
    print(f"{n:8s} rew {mm['reward']:7.0f}  thr {mm['throughput_mbps']:6.1f}  "
          f"comp {mm['completion_rate']:.3f}  miss {mm['deadline_miss_rate']:.3f}  "
          f"retx {mm['retx_drop_rate']:.4f}  depth {mm['mu_depth']:.2f}  "
