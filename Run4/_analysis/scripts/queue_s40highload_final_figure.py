...
Same protocol as the Ent02 figure, for the retired S40HighLoad (best.pt@519,
...
RUN = "Run4/QueuePostRZF_S40HighLoad"
...
with open(f"{OUTDIR}/queue_s40highload_final20.csv", "w", newline="") as f:
    w = csv.writer(f)
...
fig.suptitle(f"QueuePostRZF_S40HighLoad — PPO vs baselines  (seed {show} best-margin;  "
             f"SUS thr {best_thr} in-world swept;  best.pt@{ck['update']};  "
             f"post-RZF + beta_m S40, speed U(5,40), HIGH LOAD p U(0.15,0.50))", fontsize=10)
fig.text(0.5, 0.945,
         f"K = {cfg.num_ue} UEs ({na} active)  ·  per-UE speeds (km/h, sorted): "
         + " ".join(f"{s:.1f}" for s in spd), ha="center", fontsize=6.5)
fig.tight_layout(rect=(0, 0, 1, 0.93))
png = f"{RUN}/QueueS40HighLoad_metrics_seed{show}.png"
