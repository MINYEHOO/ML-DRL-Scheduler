"""User-count zero-shot, fixed-population edition (user redesign 2026-08-06).
...
OUT = f"Run4/_analysis/OOD/results/ood_userscale_{TAG}.csv"
...
for K in KS:
    ...
    # --- stage 1: threshold selection on fresh pilot seeds ---
    cfg = make_cfg(K)
    ...
    for seed in PILOT:
        for thr in SUS_THRS:
            ...
            log(K, "pilot", f"SUS+CQI@{thr:.2f}", seed, m)
        f.flush()
    tstar = max(SUS_THRS, key=lambda t: np.mean(by[t]))
    print("  pilot sweep: " + "  ".join(f"{t:.2f}:{np.mean(by[t]):.0f}" for t in SUS_THRS)
          + f"  -> T*={tstar:.2f}", flush=True)
